...
func NewPaymentService(repo domain.PaymentRepository, logger *zap.Logger) *PaymentService {
...
func (s *PaymentService) CreatePaymentIntent(ctx context.Context, in domain.CreatePaymentInput) (*domain.Payment, *stripe.PaymentIntent, error) {
	params := &stripe.PaymentIntentParams{
		Amount:   stripe.Int64(int64(in.Amount * 100)),
	...
	p := &domain.Payment{
		ID: uuid.New(), OrderID: in.OrderID,
		StripeIntentID: intent.ID, Amount: in.Amount, Currency: "usd",
		Status: domain.StatusPending,
		CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC(),
	}
	if err := s.repo.Create(p); err != nil { return nil, nil, err }
	s.logger.Info("payment intent created", zap.String("order_id", in.OrderID))
	return p, intent, nil
}
...
func (s *PaymentService) MarkSucceeded(orderID string) error {
	...
	return s.repo.UpdateStatus(p.ID, domain.StatusSucceeded)
}

func (s *PaymentService) MarkFailed(orderID string) error {
	p, err := s.repo.FindByOrderID(orderID)
	if err != nil { return err }
	return s.repo.UpdateStatus(p.ID, domain.StatusFailed)
}
