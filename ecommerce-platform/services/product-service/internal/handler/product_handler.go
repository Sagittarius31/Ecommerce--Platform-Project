...
import (
	...
	"github.com/yourname/ecommerce/product-service/internal/service"
	"go.uber.org/zap"
)
...
func (h *ProductHandler) ListProducts(c *gin.Context) {
	...
	if s := c.Query("page_size"); s != "" { if v, e := strconv.Atoi(s); e == nil && v <= 100 { f.PageSize = v } }
	if q := c.Query("search");    q != "" { f.Search = q }
	...
	c.JSON(http.StatusOK, gin.H{"data": products, "total": total, "page": f.Page})
}
...
func (h *ProductHandler) GetProduct(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) { c.JSON(http.StatusNotFound, gin.H{"error": "not found"}); return }
...
func (h *ProductHandler) CreateProduct(c *gin.Context) {
	...
	if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": "create failed"}); return }
...
func (h *ProductHandler) DeleteProduct(c *gin.Context) {
	id, err := uuid.Parse(c.Param("id"))
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid id"}); return }
	if err := h.svc.Delete(c.Request.Context(), id); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": "delete failed"}); return }
